...
const Attempts = ({ attempts }: { attempts: Job["attempts"] }) => {
  return (
    attempts &&
    attempts.map((attempt, i) => <Attempt key={i} attempt={attempt} />)
  );
};
  ...
  return (
    <Paper elevation={1} className="attempt" sx={{ p: "8px", mb: 1 }}>
      ...
      >
        <Typography
          variant="subtitle1"
          color={status === "failed" ? "error" : "action"}
        >
          {status}
        </Typography>
        <Stack
        ...
        >
          ...
          <Typography minWidth="100px" variant="subtitle2" color="gray" noWrap>
            {started_at ? (
              <Chrono fromNow>{started_at.toString()}</Chrono>
            ) : (
              "Pending"
            )}
          </Typography>
    ...
    <Alert severity="error">
      ...
      <div role="alert">
        <Stack direction="column">
          {stacktrace &&
            stacktrace.map((trace, i) => <Trace key={i} trace={trace} />)}
        </Stack>
      </div>
    </Alert>
  );
